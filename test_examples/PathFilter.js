/**
 * Created by dsar941 on 1/16/2016.
 */

var RequiredNodeList = [1];
...
for (var m = 0; m < RequiredNodeList.length; m++) {
    ...
    temp = dfs(RequiredNodeList[m], graph, 21);
    //temp = dfs(1, graph, 3);
    ...
    //for (var it = g.edges(), kv; !(kv = it.next()).done;) {
    //    var from = kv.value[0],
    //        to = kv.value[1],
    //        value = kv.value[2];
...
var graph = new Graph();
for (var i = 0; i < EdgeList.length; i++) {
    graph.createEdge(EdgeList[i][0], EdgeList[i][1], EdgeList[i][2]);
    graph.createEdge(EdgeList[i][1], EdgeList[i][0], EdgeList[i][2]);
}
